...
c2.execute(
    "SELECT id, conversation, conversation_ja FROM airoboros_31 WHERE conversation_ja IS NOT NULL"
)
rows = c2.fetchall()
for row in tqdm(rows):
    ...
    if any(["日本" in turn["value"] for turn in conv]):
        continue
    c.execute(
        "SELECT id, conversation_ja FROM ultraboros WHERE conversation = ?",
        (conversation,),
    )
    row2 = c.fetchone()
    if row2:
        if valid:
            c.execute(
                ...
                (conversation_ja, row2[0]),
            )
            conn.commit()
    else:
        if valid:
            c.execute(
                "INSERT INTO ultraboros (id, source, conversation, conversation_ja) VALUES (?, ?, ?, ?)",
                (id_, "airoboros", conversation, conversation_ja),
            )
        else:
            c.execute(
                "INSERT INTO ultraboros (id, source, conversation) VALUES (?, ?, ?)",
                (id_, "airoboros", conversation),
            )
        conn.commit()
