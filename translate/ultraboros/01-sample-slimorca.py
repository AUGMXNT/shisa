...
def sample_slimorca(conn):
    ...
    with open('slimorca/slim.jsonl', 'w') as outfile:
        for item in ds['train']:
            ...
            first_response = list(filter(lambda x: x.get('from', x.get('role')) in ('gpt', 'assistant'), item['conversations']))[0]
            outfile.write(json.dumps({
                'id': str(uuid.uuid5(uuid.NAMESPACE_OID, val)),
                'conversations': item['conversations'],
                'text': first_response.get("value", first_response.get("content")),
            }) + "\n")
    ...
    dsir.resample(out_dir="orca-sampled", num_to_sample=10000, cache_dir=".cache/resampled")
    ...
    c = conn.cursor()
    for item in slim_orca:
        c.execute("SELECT id FROM ultraboros WHERE id = ?", (item["id"],))
        row = c.fetchone()
        if not row:
            c.execute("INSERT INTO ultraboros (id, source, conversation) VALUES (?, ?, ?)", (item["id"], "slimorca", json.dumps(item['conversations'])))
            conn.commit()
...
""")
    ...
    c.execute("SELECT id, conversation FROM ultraboros WHERE source = 'slimorca' AND conversation_ja IS NULL")
    rows = c.fetchall()
    if not rows:
        return
    ...
    async with aiohttp.ClientSession() as client:
        for item in rows:
            ...
            tasks.append(asyncio.create_task(translate_turns(conn, item[0], conv, client)))
        if tasks:
